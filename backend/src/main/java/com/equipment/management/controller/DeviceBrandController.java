...
@RestController
@RequireAuth
@CrudPermission(module = "system:brand")
@RequestMapping("/api/device-brand")
public class DeviceBrandController extends TypedDictCrudController {

    public DeviceBrandController(DictService dictService) {
        super(dictService);
    }

    @Override
    protected String dictType() {
        return DictTypeConstants.DEVICE_BRAND;
    }
}
